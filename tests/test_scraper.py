...
def mock_env_vars(monkeypatch):
    ...
    monkeypatch.setenv(
        "TELEGRAM_CHANNELS", "chemed,lobeliacosmetics,tikvahpharma"
    )
...
def scraper(mock_env_vars, tmp_path):
    ...
    with patch('src.scraper.TelegramClient') as mock_client:
...
class TestTelegramScraper:
    ...
    @patch('src.scraper.TelegramClient')
    def test_init_with_valid_env(self, mock_client, mock_env_vars):
    ...
    def test_init_missing_api_id(self, monkeypatch):
        ...
        monkeypatch.setenv("TELEGRAM_API_HASH", "test_hash")
        with pytest.raises(
            ValueError, match="TELEGRAM_API_ID and TELEGRAM_API_HASH"
        ):
            TelegramScraper()
    ...
    def test_process_message(self, scraper, sample_message):
        ...
        assert (
            result["message_text"]
            == "Test message content with product information"
        )
    ...
    def test_process_message_with_image(
        self, scraper, sample_message_with_image
    ):
        """Test message processing with image."""
        channel_name = "test_channel"
        result = scraper._process_message(
            sample_message_with_image, channel_name
        )
    ...
    async def test_download_image(
        self, scraper, sample_message_with_image, tmp_path
    ):
        ...
        result = await scraper._download_image(
            sample_message_with_image, channel_name, images_dir
        )
    ...
    @pytest.mark.asyncio
    async def test_download_image_no_media(
        self, scraper, sample_message, tmp_path
    ):
        ...
        result = await scraper._download_image(
            sample_message, channel_name, images_dir
        )
